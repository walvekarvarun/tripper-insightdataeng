...
def main():
    conf = SparkConf().setAppName("readfroms3").set("spark.executor.memory", "6g").setMaster("spark://10.0.0.29:7077")
    sc = SparkContext(conf=conf)
    spark = SparkSession.builder.getOrCreate()
    yelpdf = read_and_filter_yelp(spark)
    bnbdf = read_and_filter_bnb(spark)
